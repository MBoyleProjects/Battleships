...
public class Board {
	...
	public static void main(String[] args) {
		allShips.add(destroyer); allShips.add(scout); allShips.add(submarine); allShips.add(launchPad); allShips.add(warship); allShips.add(paddleboat); 
	...
	//first we have a method that creates the board and fills it with empty ocean (-1)
	 public static void createBoard(){  //this method initialises/creates the board game with user input size using scanner
	 ...
	 try {
	    ...
	    if (nextInput <= 20 && nextInput > 0) {
	     ...
	     board = new int[boardHeight][boardWidth];  //here, we initialise the board variable from earlier with our scanner input values.
		 for(int row=0 ; row < boardHeight ; row++ ) {
	            for(int column=0 ; column < boardWidth ; column++) {
	                board[row][column]= EMPTY;          //this sets the value in each coordinate to be empty.
	            }
		 }
		 scanner.close();
		 }
	    	 ...
	    	 System.out.println("Sorry, please enter an integer for both the height and width, maximum of 20! Try again below:\n");
	    	 createBoard();
	     }
	    } 
	 ...
	 public static void showBoard(int[][] board){  
		...
		char[] alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".toCharArray();
	     ...
	     System.out.println();
	        ...
	        for(int row=0 ; row < boardHeight ; row++ ){      //for each row print out the number of the row+1
	            System.out.print((alphabet[row])+"");
	            
	            for(int column=0 ; column < boardWidth ; column++ ){    //for every column, print either ~, * or X depending on the game state.
	...
	//this method recommends the number of ships that you should place on the board, given its dimensions
	 public static int recommendedShipNo(int boardheight, int boardwidth) {
		 int boardsize = boardheight*boardwidth;
		 
		 if (boardsize <= 15)  return 1;
		 if (boardsize > 15 && boardsize <= 40) return 2;
		 if (boardsize > 40 && boardsize <= 60) return 3;
		 if (boardsize > 60 && boardsize <= 80) return 4;
		 if (boardsize > 80 && boardsize <= 100) return 5;
		 else return 6;
	 }
	 
	 
	//this method allows the user to choose from a selection of ships, which ones they want to place on their board 
	 public static void chooseShips() {
		...
		// System.out.println("Please type in your selected ships by number, separated by commas.");
		 
		 if (numberOfShips == 2){
			 chosenShips.add(warship); chosenShips.add(scout);
		 }
		 if (numberOfShips == 3){
			 chosenShips.add(warship); chosenShips.add(paddleboat); chosenShips.add(submarine);
		 }
		   if (numberOfShips == 4){
			   chosenShips.add(launchPad); chosenShips.add(paddleboat); chosenShips.add(scout); chosenShips.add(destroyer);
		   }
		 if (numberOfShips == 5) {
			 chosenShips.add(destroyer); chosenShips.add(submarine); chosenShips.add(scout); chosenShips.add(warship); chosenShips.add(paddleboat);
		 }
		 if (numberOfShips == 6){
			 chosenShips.addAll(allShips);
		 }
		 
		 //ArrayList<Ship> chosenships = new ArrayList<Ship>(); 	//	 int shipadds = 0;
...
//
		 ...
		 public static ArrayList<String> placeShips(Ship ship) {
			...
			int x = 0;                                        // nth position in ship to place (ie if all ships are size 3, x should never go above 3
		        ...
		        while (success && x < shipSize) {                // look for adjacent unused spots
		          if (grid[location] == 0) {                    // if not already used
		             ...
		             if (x>0 & (location % boardWidth == 0)) {  // out of bounds - right edge        
		               success = false;                         // failure
		             }
		  ...
		  public static String getUserGuess() { //this is just a basic user input class
